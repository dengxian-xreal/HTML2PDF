...
def main():
    driver = setup_driver()
    try:
        base_url = "https://xreal.gitbook.io/nrsdk" # change url
        driver.get(base_url)
        time.sleep(10)  # Wait for the page to fully load
        elements = driver.find_elements(By.CSS_SELECTOR, "a")
        links = {element.get_attribute('href') for element in elements if 'https://xreal.gitbook.io' in (element.get_attribute('href') or '')} #change url
        for link in links:
